...
const GuestReview = () => {
  ...
  const fetchReviews = async (page: number, searchVal: string | null) => {
    try {
      setLoading(true);
      const response = await getUserReviewList(page, searchVal);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await deleteGuestReview(deleteReviewId);
      if (response.status === 200) {
  ...
  const columns = [
    {
      ...
      header: "Guest",
      cell: (review: Review) => (
        <div className="font-medium">{capitalizeWord(review.guest)}</div>
      ),
    },
    {
      key: "property",
    ...
    <DashboardLayout title="Reviews">
      <div className="space-y-6">
        <DataTable
          ...
          columns={columns}
          searchable
          searchPlaceholder="Search by guest name or reviewer..."
