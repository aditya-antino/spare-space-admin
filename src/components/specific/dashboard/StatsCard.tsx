import { LucideIcon } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Skeleton } from "@/components/ui/skeleton";

interface StatsCardProps {
  title: string;
  value: string;
  icon: LucideIcon;
  onClick?: () => void;
  loading?: boolean;
  error?: boolean;
  description?: string;
}

const DashboardStatusCard = ({
  title,
  value,
  icon: Icon,
  onClick,
  loading = false,
  error = false,
  description,
}: StatsCardProps) => {
  if (loading) {
    return (
      <Card className="border-secondary-s2 bg-secondary-s1 h-[112px]">
        <CardContent className="p-5 flex items-center justify-between h-full">
          <div className="space-y-1 flex-1">
            <Skeleton className="h-4 w-20 bg-secondary-s2" />
            <Skeleton className="h-8 w-16 bg-secondary-s2" />
            <Skeleton className="h-3 w-24 bg-secondary-s2" />
          </div>
          <Skeleton className="h-12 w-12 rounded-lg bg-secondary-s2 shrink-0" />
        </CardContent>
      </Card>
    );
  }

  if (error) {
    return (
      <Card className="border-danger-tint1 bg-danger-tint1/10 h-[112px]">
        <CardContent className="p-5 flex items-center justify-between h-full">
          <div className="space-y-1">
            <p className="text-sm font-medium text-danger-d1">{title}</p>
            <h3 className="text-2xl font-bold text-danger-d1">Error</h3>
            <p className="text-xs text-danger-d1">Failed to load</p>
          </div>
          <div className="p-3 bg-danger-tint1/20 rounded-lg shrink-0">
            <Icon className="h-6 w-6 text-danger-d1" />
          </div>
        </CardContent>
      </Card>
    );
  }

  return (
    <Card
      className="cursor-pointer border-secondary-s2 bg-secondary-s1 hover:shadow-md transition-shadow h-[112px]"
      onClick={onClick}
    >
      <CardContent className="p-5 flex items-center justify-between h-full">
        <div className="space-y-1">
          <p className="text-sm font-medium text-tertiary-t3">{title}</p>
          <h3 className="text-2xl font-bold text-tertiary-t1">{value}</h3>
          {description && (
            <p className="text-[10px] text-tertiary-t3 font-normal opacity-75 leading-none mt-0.5">
              {description}
            </p>
          )}
        </div>
        <div className="p-3 bg-primary-tint4 rounded-lg shrink-0">
          <Icon className="h-6 w-6 text-primary-p3" />
        </div>
      </CardContent>
    </Card>
  );
};

export default DashboardStatusCard;
